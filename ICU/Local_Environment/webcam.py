...
def capture_and_send():
    ...
    while True:
        ...
        try:
            # AI 서버로 프레임 전송
            print(f"Sending frame of size {len(frame_bytes)} bytes to {AI_SERVER_URL}")
            response_ai = asyncio.get_event_loop().run_until_complete(
                send_frame_to_server(AI_SERVER_URL, frame_bytes))
            print(f"Received response from AI server: {response_ai}")

            # 웹 서버로 프레임 전송
            print(f"Sending frame of size {len(frame_bytes)} bytes to {WEB_SERVER_URL}")
            response_web = asyncio.get_event_loop().run_until_complete(
                send_frame_to_server(WEB_SERVER_URL, frame_bytes))
